...
export async function POST(req: Request) {
  try {
    ...
    if (!team) {
      return NextResponse.json({ error: 'Email not found. Please complete previous levels first.' }, { status: 404 });
    }

    const otp = Math.floor(100000 + Math.random() * 900000).toString();
    await setOTP(`xenith:l3:verify:${email}`, otp);
    await sendXenithOTP({ email, otp, level: 3 });
    ...
    console.error('Level 3 send OTP error:', error);
